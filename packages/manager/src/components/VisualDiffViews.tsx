          ...
          <ul className="bb-vdiff__rows">
            {node.props.map((p) => (
              <li key={p.prop} className="bb-vdiff__row">
                ...
                {p.isColor ? (
                  <ColorSwatch value={p.from} />
                ) : (
                  <span className="bb-vdiff__mono bb-vdiff__from">{p.from}</span>
                )}
                <span className="bb-vdiff__arrow" aria-hidden="true">
                  →
                </span>
                {p.isColor ? (
                  <ColorSwatch value={p.to} />
                ) : (
                  <span className="bb-vdiff__mono bb-vdiff__to">{p.to}</span>
                )}
              </li>
            ))}
          </ul>
...
export function DomDiffView({ diff }: { diff: DomDiff }) {
  const total = diff.added.length + diff.removed.length + diff.moved.length + diff.changed.length;
              ...
              <li className="bb-vdiff__row">
                ...
                <span className="bb-vdiff__mono">
                  {c.classes.removed.map((cl) => (
                    <span key={cl} className="bb-vdiff__chip" data-kind="removed">
                      −{cl}
                    </span>
                  ))}
                  {c.classes.added.map((cl) => (
                    <span key={cl} className="bb-vdiff__chip" data-kind="added">
                      +{cl}
                    </span>
                  ))}
                </span>
          ...
          <span className="bb-vlayout__chips">
            {layoutChips(n).map((c, i) => (
              <span key={i} className="bb-vlayout__chip" data-axis={c.axis} data-dir={c.dir}>
                <span className="bb-vlayout__chip-icon" aria-hidden="true">
                  {c.icon}
                </span>
    ...
    <ul className="bb-vdiff__a11y">
      {diff.changes.map((c, i) => (
        <li key={i} className="bb-vdiff__a11y-line" data-kind={c.kind}>
          <span className="bb-vdiff__a11y-sign" aria-hidden="true">
            {c.kind === "added" ? "+" : "−"}
          </span>
